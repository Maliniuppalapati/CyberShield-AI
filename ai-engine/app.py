...
def analyze_email():
    ...
    if not GROQ_API_KEY or GROQ_API_KEY.startswith('gsk_YOUR'):
        return jsonify({
            "isPhishing": False,
            "confidence": 0,
            "analysis": "AI Engine is offline or missing API key. Cannot analyze email.",
            "threats": []
        })

    try:
        import requests
        system_instruction = """You are a Tier 3 SOC Analyst specializing in email forensics and phishing detection.
Analyze the provided email content (headers and body if available).
Respond with a strict JSON object (NO markdown formatting, just the raw JSON string) containing exactly these fields:
- "isPhishing": boolean (true if malicious/phishing, false if safe/benign)
- "confidence": number between 0 and 100 representing your confidence
- "analysis": string explaining your reasoning in 2-3 sentences
- "threats": array of strings listing detected tactics (e.g. ["Urgency", "Spoofed Sender", "Suspicious Link"]) or empty if none."""
        ...
        if response.status_code == 200:
            ...
            try:
                import json
                # Sometimes LLMs wrap JSON in markdown block, strip it
                ...
                return jsonify({
                    "isPhishing": True,
                    "confidence": 50,
                    "analysis": f"Analysis completed but failed to parse structured output. Raw response: {reply_text}",
